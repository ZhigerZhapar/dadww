...
const Categories = ({ activeCategory, onCategoryClick, handleFilterPageClose }) => {
    ...
    const [activeCategoryId, setActiveCategoryId] = useState(initialCategoryId || activeCategory);
    ...
    }, [activeCategory, data.data, dispatch]);

    useEffect(() => {
        if (!activeCategory && data.data && data.data.length > 0) {
            const defaultActiveCategory = data.data[0].id;
            setActiveCategoryId(defaultActiveCategory);
        } else if (activeCategory && data.data && data.data.length > 0 && selectedButton === null) {
            const defaultActiveCategory = data.data.find((button) => button.id === activeCategory);

            if (defaultActiveCategory) {
                const defaultActiveCategoryIndex = data.data.indexOf(defaultActiveCategory);
                setSelectedButton(defaultActiveCategoryIndex);
                setActiveCategoryId(defaultActiveCategory.id);

                if (onCategoryClick) {
                    onCategoryClick({
                        category: defaultActiveCategory,
                        categoryId: defaultActiveCategory.id,
                    });
                }
            }
        }
    }, [activeCategory, data.data, onCategoryClick, selectedButton]);
    useEffect(() => {
